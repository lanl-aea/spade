...
include_h5_objects = h5_file.o
include_h5_sources = h5_file.cpp
...
link_exe = $(GPP) -fPIC -Wl,-Bdynamic -Wl,--add-needed -o %J %F %M %L %B %O -L$(H5_PATH)/lib -lhdf5 -lhdf5_cpp -L$(GPP_PATH)/lib64 -L$(GPP_PATH)/lib -lstdc++
...
$(tool): $(include_local_objects) $(abq_env)
